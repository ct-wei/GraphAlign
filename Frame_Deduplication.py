...
class PPTKeyframeExtractor:
    def __init__(self, video_path, output_dir="output"):
        ...
        # 优化参数配置（可根据视频特性调整）
        self.config = {
            'base_threshold': 15000,  # 幻灯片切换检测阈值
            'accum_threshold': 60000,  # 动画累积变化阈值
            'min_segment_len': 30,  # 最短分段长度（帧数）
    ...
    def process(self):
        """完整处理流程"""
        # 1. 视频采帧
        frames = self.extract_frames(interval=10)
...
if __name__ == "__main__":
    ...
    processor = PPTKeyframeExtractor(
        video_path="/home/zyserver/SSM/SSM/video/com/TXYL41.mp4",
        output_dir="/home/zyserver/SSM/SSM/video/output"
    )
